...
function elCanvas(container, props) {
	...
	this.translateScaleRotate = function(props,parentLevel) {
		// get center points
			...
			centerX = props.width * (Number(tO[0]) / 100),
			centerY =  props.height * (Number(tO[1]) / 100)
			...
			// below is for centering the element if centered is on
		...
		svg: function(p) {
			...
			*/
			if(p.__cachedPoints!==p.points){
				...
				p.__cachedFunction = el._util.parseSvgPathData(p.points,el.ctx);
	...
	this._util = {
       ...
       */
		...
		parseSvgPathData:function(data,ctx) {
			var calls = [];
		  var ifc = {
		      init : function() {
