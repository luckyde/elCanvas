	...
	elCanvas.prototype = {
		...
		renderFuncs: {
			...
			shape: function (el, props) {
				...
				func(el);
				el.fillAndStroke(props, props.opacity);
			},
		...
		remove: function (objOrArray) {
			/*
			Something about this version is messing up on daily puzzle,
			and I'm not sure what but letters dissapear(they're children of the tiles if that helps)
			Glenn updated version:
			 ...
			 */
			var objectIndex = this.objects.indexOf(objOrArray);
			if(objOrArray instanceof Array){
				for(var i=0;i<objOrArray.length;i++){
					this.remove(objOrArray[i])
				}
			}else if(objectIndex>-1){
				// remove children
				if(objOrArray.childNodes){
					for(var i=0;i<objOrArray.childNodes.length;i++){
						var childObject = objOrArray.childNodes[i];
