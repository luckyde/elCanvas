...
elCanvas = (function () {
	var objectTypes = ["rect", "circle", "line", "quadLine", "image", "shape", "text", "svg" ];
	var defaultStyles = {
		fallback: {
			lineWidth: 0.001,
	...
	elCanvas.prototype = {
		...
		prepFuncs: {
			...
			image: function (props) {
				...
				if (props.textProps) {
					props.width = props.textProps.w;
					props.height = props.textProps.h;
				}
			}
		},
		...
		renderFuncs: {
			...
			image: function (el, props) {
				...
				if (props.textProps) {
					// textSprite option
					el.ctx.drawImage(image,  props.textProps.x,  props.textProps.y, props.textProps.w, props.textProps.h,
						0, 0, props.textProps.w * props.textProps.scale, props.textProps.h * props.textProps.scale);
				} else {
					// normal image option
					if (props.rounded) {
	...
	objectTypes.forEach(function (objectType) {
		elCanvas.prototype[objectType] = function (suppliedProps) {
			suppliedProps = _util.mergeDefaultProperties(suppliedProps, objectType);
			return this.addObject(suppliedProps);
		}
	});
