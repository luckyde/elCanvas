...
elCanvas = (function () {
	...
	var _util = {
		 ...
		 */
		generateRadialGradientBitmap: function (el, id, size, innerColour, outerColour) {
	...
	elCanvas.prototype = {
      ...
      Array.prototype.push.apply(el.imagesToLoad, props);
		},
      ready:function(newLoadedImages){
        //merge loaded images with general images object
        Object.assign( el.images,newLoadedImages);
        //clear images to loead
        el.imagesToLoad=[]; 
        el.readyFunction();
    },
    onready: function(callback){
    // console.log('aand',callback);
      // console.log(callback);
      el.readyFunction = callback;
    },
		...
		debugObject: function (props) {
			if (props.debug) {
				...
				this.ctx.set("globalCompositeOperation", 'difference').fill();
			}
		},
        ...
        if(props.url){
          var newID = props.id ? props.id:'unnamed_'+ el.objects.length;
          props.id = newID;
          if(!el.images[props.id]){
            el.imagesToLoad.push({id:props.id,url:props.url})
          }
				...
				if (!this.images[props.id]) {
					...
					var img = new Image();
					img.src = defaultStyles.image.src;
					img.id = 'backup';
          props.isBackupImage=true;
					defaultStyles.image.object = img; // TODO
				}
			},
		...
		prepFuncs: {
			...
			image: function (props) {
				...
				var image = this.images[props.id] || defaultStyles.image.object;
		...
		renderFuncs: {
			...
			image: function (el, props) {
				...
				var image = el.images[props.id] || defaultStyles.image.object;
        //the image is loaded
        if(el.images[props.id]){
          //if we're using the backup image still
          if(props.isBackupImage){
            props.isBackupImage=false;
            // update the data so the width and height are accurate for scaling them
      ...
      // if there are images to load and its not already loading them, load them
      if(el.imagesToLoad && !el.imageLoadingInProgress){
          el.imageLoadingInProgress=true;
          el_imageLoad(el.imagesToLoad,el.ready)
      }
      ...
      this.firstFrame = false;
		...
		hitTest: function (mousePosition, callback) {
			var found = this.objects.find(function (object) {
				return object.interactable && _util.fullyInside(mousePosition, object);
			});
			found && callback && callback(found);
		},
