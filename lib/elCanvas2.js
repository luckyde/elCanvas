...
elCanvas = (function () {
	...
	elCanvas.prototype = {
      ...
      el.imagesLoaded=false;
      Array.prototype.push.apply(el.imagesToLoad, props);
		},
    mergeImageAssets:function(newLoadedImages){
      ...
      el.readyFunction();
      el.imagesLoaded=true;
    },
    onReady:function(callback){
      // if there are images to load and its not already loading them, load them
      if(el.imagesToLoad.length>0){
        el.imagesLoaded=false;
        el_imageLoad(el.imagesToLoad,el.mergeImageAssets);
        // assign a callback function so we can animate once ready
        ...
        if(props.url){
          ...
          if(!el.images[props.id]){
            ...
            el_imageLoad(el.imagesToLoad,function(){});
          }
