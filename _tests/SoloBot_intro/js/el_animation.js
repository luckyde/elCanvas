function el_animation(el, params){
  ...
  var bodyNull= new el.rect({x:80,y:220,width:140,height:140,transformOrigin:"50% 100%",style:{fillStyle:"rgba(0,0,0,0)"}})

  var jacket = new el.image({id:'jacket',zIndex:3,parent:bodyNull,transformOrigin:"50% 80%"})

  var belt = new el.image({x:23,y:-3,zIndex:-1,id:'belt',parent:jacket})
  var cuffs = new el.image({x:13,y:6,zIndex:2,id:'cuffs',parent:jacket})
  var tie = new el.image({x:28,y:3,zIndex:5,id:'tie',parent:jacket})

  var head = new el.image({zIndex:3,id:'head',x:-47,y:-195,parent:jacket,transformOrigin:"50% 80%"})
  var mouth = new el.image({x:93,y:162,id:'mouth',parent:head})
      ,mustache = new el.image({x:74,y:149,id:'mustache',parent:head})
      ,brow_l = new el.image({x:33,y:85,id:'brow_l',parent:head})
      ,eye_r = new el.image({x:152,y:132,id:'eye_r',parent:head})
      ,eye_l = new el.image({x:72,y:134,id:'eye_l',parent:head})
      ,glasses_r = new el.image({x:137,y:106,id:'glasses_r',parent:head})
      ,nose = new el.image({x:95,y:122,id:'nose',parent:head})
      ,glasses_l = new el.image({x:35,y:108,id:'glasses_l',parent:head})

      var arm_upper_l = new el.image({x:-40,y:10,zIndex:1,id:'arm_upper_l',parent:jacket,transformOrigin:"98% 9%"})
      var arm_lower_l = new el.image({x:-3,y:35,zIndex:2,id:'arm_lower_l',parent:arm_upper_l,transformOrigin:"23% 22%"})
      var hand_l = new el.image({x:23,y:39,zIndex:-1,id:'hand_l',parent:arm_lower_l,transformOrigin:"50% 50%"})

      var arm_r = new el.image({x:100,y:5,zIndex:0,id:'arm_r',parent:bodyNull,transformOrigin:"0% 34%"})
      var hand_r = new el.image({x:48,y:-26,zIndex:2,id:'hand_r',parent:arm_r,transformOrigin:"39% 89%"})
